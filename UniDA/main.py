...
def train(args):
    ...
    for i in range(args.max_iterations + 1):
        ...
        if i>=args.start_unida:
            fc = copy.deepcopy(base_network.fc)
            for param in fc.parameters():
                param.requires_grad = False
            if (i - args.start_unida) % len(dset_loaders["target"]) == 0:
            ...
            try:
                inputs_target, labels_target, ids_target = iter_target.__next__()
            except:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dset', type=str, default='office_home',
                        choices=["office_home"])
    parser.add_argument('--root', type=str, default='/public/home/guxiang/dataset',help="root to data")
    ...
    data_folder = './data/'
    args.t_dset_path = data_folder + args.dset + '/target_' + names[args.t] + '_univ.txt'
    args.s_dset_path = data_folder + args.dset + '/source_' + names[args.s] + '_univ.txt'
